...
class CUGAN_stack(nn.Module):
    def __init__(self, in_nc, out_nc, cond_dim, stages_blocks_num, stages_channels, downSample_Ksize):
        ...
        self.convfusion = nn.Conv2d(in_nc, out_nc, kernel_size=3, stride=1, padding=1, bias=True)
        # stage1 left
        self.conv_stage1_left = nn.Conv2d(out_nc, self.stage1_nf, kernel_size=3, stride=1, padding=1, bias=True)
    ...
    def forward(self, src,input_cond,output_cond):
        content, input_cond,output_cond = src,input_cond,output_cond

        content = self.convfusion(content)
        # stage1 left
        out = self.conv_stage1_left(content)
        stage1_left, _ = self.CResBlocks_stage1_left((out, input_cond))

        # stage2 left
        out = self.conv_stage2_left(stage1_left)
        stage2_left, _ = self.CResBlocks_stage2_left((out, input_cond))

        # stage3 left
        out = self.conv_stage3_left(stage2_left)
        stage3_left, _ = self.CResBlocks_stage3_left((out, input_cond))


        # stage3 right
        stage3_right, _ = self.CResBlocks_stage3_right((stage3_left, output_cond))
        stage3_right = self.conv_stage3_right(stage3_right)
        # stage 3 Modulated Scale Fusion
        stage3_scale = self.stage3_scale(output_cond).view(-1, stage3_right.size()[1], 1, 1)
        stage2_left = stage2_left + stage3_scale * stage3_right


        # stage2 right
        stage2_right, _ = self.CResBlocks_stage2_right((stage2_left, output_cond))
        stage2_right = self.conv_stage2_right(stage2_right)
        # stage 2 Modulated Scale Fusion
        stage2_scale = self.stage2_scale(output_cond).view(-1, stage2_right.size()[1], 1, 1)
        stage1_left = stage1_left + stage2_scale * stage2_right


        # stage1 right
        stage1_right, _ = self.CResBlocks_stage1_right((stage1_left, output_cond))
        stage1_right = self.conv_stage1_right(stage1_right)
        # stage 1 Modulated Scale Fusion
        stage1_scale = self.stage1_scale(output_cond).view(-1, stage1_right.size()[1], 1, 1)
